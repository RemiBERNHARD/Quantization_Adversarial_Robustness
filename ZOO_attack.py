...
class BlackBoxL2:
    def __init__(self, sess, model, batch_size=1, confidence = CONFIDENCE,
                 ...
                 init_size = 32, use_importance = True, image_size=32, num_channels=3, num_labels=10, clip_min=0.0, clip_max=1.0):
        """
        ...
        if use_tanh:
            self.newimg = (tf.tanh(self.scaled_modifier + self.timg) + 1 ) /2
        else:
        ...
        if use_tanh:
            self.l2dist = tf.reduce_sum(tf.square(self.newimg - self.other), [1,2,3])
        else:
        ...
        if self.TARGETED:
            if use_log:
                ...
                #loss1 = tf.maximum(0.0, tf.log(other + 1e-30) - tf.log(real + 1e-30))
            else:
            ...
            if use_log:
                ...
                #loss1 = tf.maximum(0.0, tf.log(real + 1e-30) - tf.log(other + 1e-30))
            else:
    ...
    def blackbox_optimizer(self, iteration):
        ...
        #print(np.min(nimgs[0]))
        #print(np.max(nimgs[0]))
        return losses[0], l2s[0], loss1[0], loss2[0], scores[0], nimgs[0]
        # return losses[0]
    ...
    def attack_batch(self, img, lab):
        ...
        if (np.array_equal(tamp, o_bestattack) == True):
            #print("oui")
            o_bestattack = (np.tanh(o_bestattack) +1)/2
