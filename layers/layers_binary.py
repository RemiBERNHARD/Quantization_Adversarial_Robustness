...
# -*- coding: utf-8 -*-
"""
Created on Thu Jan 10 13:34:14 2019
...
def grad_fg(op, grad):
    x = op.inputs[0]
    indicatrice = tf.cast(tf.less_equal(tf.abs(x), 1), tf.float32) # alpha = (|x| <= 1) * 1
    return tf.multiply(grad, indicatrice)


def binarize_activation(x):
     g = tf.get_default_graph()
     with ops.name_scope("Binarized") as name:
        with g.gradient_override_map({"Sign": "FGGrad"}):
...
class Clip(constraints.Constraint):
    def __init__(self, min_value, max_value=None, **kwargs):
...
class BinaryDense(Dense):
    ...
    def call(self, inputs):    
        binary_inputs = binarize_activation(inputs)
...
class BinaryConv2Dweights(Conv2D):
    ...
    def __init__(self, filters, kernel_regularizer=None,activity_regularizer=None, kernel_lr_multiplier='Glorot',
        ...
        super(BinaryConv2Dweights, self).__init__(filters, **kwargs)
        self.H = H
        self.kernel_lr_multiplier = kernel_lr_multiplier
        self.bias_lr_multiplier = bias_lr_multiplier
    ...
    def get_config(self):
        config = {'H': self.H,
                  'kernel_lr_multiplier': self.kernel_lr_multiplier,
                  'bias_lr_multiplier': self.bias_lr_multiplier}
        base_config = super(BinaryConv2Dweights, self).get_config()
        return dict(list(base_config.items()) + list(config.items()))
